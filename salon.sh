...
CREATE_APPOINTMENT(){
  ...
  SERVICE_PRICE_FORMATTED=$(echo $SERVICE_PRICE | sed 's/\s//g' -E)
  ...
  echo -e "\nI have put you down for a $SERVICE_NAME_FORMATTED at $SERVICE_TIME, $CUST_NAME_FORMATTED, your total will be $SERVICE_PRICE_FORMATTED$."
}
...
LIST_SERVICES(){
  ...
  echo "$SERVICES" | while read SERVICE_ID BAR NAME BAR PRICE
  do
